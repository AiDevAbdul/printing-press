    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-lg shadow border-l-4 border-blue-500">
          ...
          <div className="text-3xl font-bold mt-2">{stats?.total_invoices || 0}</div>
        </div>

        <div className="bg-white p-6 rounded-lg shadow border-l-4 border-green-500">
          <div className="text-gray-600 text-sm font-medium">Paid</div>
          <div className="text-3xl font-bold mt-2">${stats?.paid_amount || 0}</div>
        </div>

        <div className="bg-white p-6 rounded-lg shadow border-l-4 border-yellow-500">
          <div className="text-gray-600 text-sm font-medium">Pending</div>
          <div className="text-3xl font-bold mt-2">${stats?.pending_amount || 0}</div>
        </div>

        <div className="bg-white p-6 rounded-lg shadow border-l-4 border-red-500">
          <div className="text-gray-600 text-sm font-medium">Overdue</div>
          <div className="text-3xl font-bold mt-2">${stats?.overdue_amount || 0}</div>
        </div>
      </div>

      <div className="bg-white p-6 rounded-lg shadow">
        <h2 className="text-xl font-semibold mb-4">Recent Invoices</h2>
        <p className="text-gray-600">Invoice list will be displayed here</p>
      </div>
    </div>
  );
}
